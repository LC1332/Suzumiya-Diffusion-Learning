...
# 保存ROC曲线到指定路径
output_path = 'output/openai-clip_LDA_ROC.png'
feature_column_name = 'FaRL'

# lda_model_path = 'output/lda_openai_clip_model.pkl'

lda_model_path = 'output/lda_openai_clip_model_norm.pkl'
...
for index, celeb_name in enumerate(celeb_names):
    ...
    A = project_to_lda(A) if A is not None else None
    B = project_to_lda(B) if B is not None else None
    ...
    C = project_to_lda(C) if C is not None else None
    ...
    neg_feat_sel = project_to_lda(neg_feat_sel[0])
    negative_pairs.append((C, neg_feat_sel))
    
    if False:
        print(negative_pairs[0][0].shape)
        print(positive_pairs[0][0].shape)
        break

# check size
for pair in positive_pairs:
    for k in range(2):
        if pair[k].shape[1] != 512 or len(pair[k].shape) != 2:
            print("warning positive ", pair[0].shape)
            break

for pair in negative_pairs:
    for k in range(2):
        if pair[k].shape[1] != 512 or len(pair[k].shape) != 2:
            print("warning negative ", pair[0].shape)
            break
print("check size done")
...
# for pair in positive_pairs:
#     try:
#         print(cosine_similarity([pair[0]], [pair[1]])[0][0])
#     except:
#         print("first format wrong")

#     try:
#         print(cosine_similarity(pair[0], pair[1])[0][0])
#     except:
#         print("second format wrong")
...
# # 计算正面对和负面对的欧氏距离
# positive_similarities = [-euclidean(pair[0].flatten(), pair[1].flatten()) for pair in positive_pairs]
# negative_similarities = [-euclidean(pair[0].flatten(), pair[1].flatten()) for pair in negative_pairs]
